...
SandBox::SandBox(core::Config &config) : Engine(config)
{
}
...
void SandBox::OnInit()
{
    ...
    {
        scene::LightCreationDesc pointLightcreationDesc{.lightNumber = i,
    ...
    scene::LightCreationDesc directionalLightCreationDesc{.lightNumber = DIRECTIONAL_LIGHT_OFFSET + 0u,
                                                          .lightType = scene::LightTypes::DirectionalLightData};

    mScene->AddLight(mDevice.get(), directionalLightCreationDesc);

    // Load skybox.
    scene::SkyBoxCreationDesc skyBoxCreationDesc{.equirectangularTexturePath = L"Assets/Textures/SkyOnFire.hdr",
    ...
    mPostProcessBuffer = std::make_unique<gfx::Buffer>(
        mDevice->CreateBuffer<PostProcessBuffer>(postProcessBufferCreationDesc, std::span<PostProcessBuffer, 0u>{}));
    mPostProcessBufferData = {.exposure = 1.0f};

    CreatePipelineStates();

    // Load depth stencil texture.
    gfx::TextureCreationDesc depthStencilTextureCreationDesc{.usage = gfx::TextureUsage::DepthStencil,
    ...
    gfx::TextureCreationDesc fowardDepthStencilTextureCreationDesc{.usage = gfx::TextureUsage::DepthStencil,
                                                                   .dimensions = mDimensions,
                                                                   .format = DXGI_FORMAT_R32_FLOAT,
                                                                   .name = L"Forward Depth Stencil Texture"};

    mForwardRenderingDepthStencilTexture =
        std::make_unique<gfx::Texture>(mDevice->CreateTexture(fowardDepthStencilTextureCreationDesc));

    // Load render targets.
    gfx::TextureCreationDesc offscreenRenderTargetTextureCreationDesc{.usage = gfx::TextureUsage::RenderTarget,
                                                                      .dimensions = mDimensions,
                                                                      .format = DXGI_FORMAT_R16G16B16A16_FLOAT,
                                                                      .name = L"Offscreen Render Texture"};

    mOffscreenRT =
        std::make_unique<gfx::RenderTarget>(mDevice->CreateRenderTarget(offscreenRenderTargetTextureCreationDesc));

    gfx::TextureCreationDesc postProcessRenderTargetsTextureCreationDesc{.usage = gfx::TextureUsage::RenderTarget,
                                                                         .dimensions = mDimensions,
                                                                         .format = gfx::Device::SWAPCHAIN_FORMAT,
                                                                         .name = L"Post Process Render Texture"};

    mPostProcessingRT =
        std::make_unique<gfx::RenderTarget>(mDevice->CreateRenderTarget(postProcessRenderTargetsTextureCreationDesc));

    gfx::TextureCreationDesc finalRenderTargetsTextureCreationDesc{.usage = gfx::TextureUsage::RenderTarget,
                                                                   .dimensions = mDimensions,
                                                                   .format = gfx::Device::SWAPCHAIN_FORMAT,
...
{
    std::unique_ptr<gfx::GraphicsContext> deferredGPassGraphicsContext =
        mDevice->GetGraphicsContext(mDeferredGPass->mDeferredPassPipelineState.get());
    std::unique_ptr<gfx::GraphicsContext> shadingGraphicsContext = mDevice->GetGraphicsContext(mPBRPipelineState.get());
    std::unique_ptr<gfx::GraphicsContext> postProcessingGraphicsContext =
        mDevice->GetGraphicsContext(mPostProcessingPipelineState.get());
    ...
    std::array<const gfx::RenderTarget *, 1u> renderTargets{mOffscreenRT.get()};
    ...
    // Renderpass 0 : Deferred Geometry pass
    {
        ...
        deferredGPassGraphicsContext->CopyResource(mDepthStencilTexture->GetResource(),
                                                   mForwardRenderingDepthStencilTexture->GetResource());

        deferredGPassGraphicsContext->AddResourceBarrier(
            mDepthStencilTexture->GetResource(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_DEPTH_WRITE);
        deferredGPassGraphicsContext->AddResourceBarrier(mForwardRenderingDepthStencilTexture->GetResource(),
                                                         D3D12_RESOURCE_STATE_COPY_DEST,
                                                         D3D12_RESOURCE_STATE_DEPTH_WRITE);
    ...
    // render lights using forward rendering.
    {
        ...
        DeferredLightingPassRenderResources deferredLightingPassRenderResources{
            ...
            .albedoGBufferIndex =
                gfx::RenderTarget::GetRenderTextureSRVIndex(mDeferredGPass->mDeferredPassRTs.albedoRT.get()),
            .positionEmissiveGBufferIndex =
                gfx::RenderTarget::GetRenderTextureSRVIndex(mDeferredGPass->mDeferredPassRTs.positionEmissiveRT.get()),
            .normalEmissiveGBufferIndex =
                gfx::RenderTarget::GetRenderTextureSRVIndex(mDeferredGPass->mDeferredPassRTs.normalEmissiveRT.get()),
            .aoMetalRoughnessEmissiveGBufferIndex = gfx::RenderTarget::GetRenderTextureSRVIndex(
                mDeferredGPass->mDeferredPassRTs.aoMetalRoughnessEmissiveRT.get()),
        ...
        shadingGraphicsContext->AddResourceBarrier(renderTargets, D3D12_RESOURCE_STATE_RENDER_TARGET,
    ...
    // processing has occured).
    {
        ...
        postProcessingGraphicsContext->SetRenderTarget(mPostProcessingRT.get(),
                                                       mForwardRenderingDepthStencilTexture.get());
        ...
        postProcessingGraphicsContext->ClearRenderTargetView(mPostProcessingRT.get(),
    ...
    // For now, UI is rendered in this RT as well.
    {
        finalGraphicsContext->AddResourceBarrier(mPostProcessingRT->GetResource(), D3D12_RESOURCE_STATE_RENDER_TARGET,
                                                 D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        finalGraphicsContext->AddResourceBarrier(mFinalRT->GetResource(), D3D12_RESOURCE_STATE_COPY_SOURCE,
                                                 D3D12_RESOURCE_STATE_RENDER_TARGET);
        ...
        mEditor->Render(mDevice.get(), mScene.get(), &mDeferredGPass->mDeferredPassRTs, clearColor,
                        mPostProcessBufferData, mPostProcessingRT.get(), finalGraphicsContext.get());
    }

    // Render pass 3 : Copy the final RT to the swap chain
    {
        finalToSwapChainGraphicsContext->AddResourceBarrier(mFinalRT->GetResource(), D3D12_RESOURCE_STATE_RENDER_TARGET,
                                                            D3D12_RESOURCE_STATE_COPY_SOURCE);
        finalToSwapChainGraphicsContext->AddResourceBarrier(backBuffer->GetResource(), D3D12_RESOURCE_STATE_PRESENT,
                                                            D3D12_RESOURCE_STATE_COPY_DEST);
        ...
        finalToSwapChainGraphicsContext->AddResourceBarrier(backBuffer->GetResource(), D3D12_RESOURCE_STATE_COPY_DEST,
    ...
    std::array<gfx::GraphicsContext *, 5> graphicsContexts{
        deferredGPassGraphicsContext.get(), shadingGraphicsContext.get(),          postProcessingGraphicsContext.get(),
        finalGraphicsContext.get(),         finalToSwapChainGraphicsContext.get(),
    };
...
void SandBox::CreatePipelineStates()
{
    ...
    mPostProcessingPipelineState = std::make_unique<gfx::PipelineState>(
        mDevice->CreatePipelineState(postProcessGraphicsPipelineStateCreationDesc));
    ...
    mPBRPipelineState =
        std::make_unique<gfx::PipelineState>(mDevice->CreatePipelineState(pbrPipelineStateCreationDesc));
    ...
    mLightPipelineState =
        std::make_unique<gfx::PipelineState>(mDevice->CreatePipelineState(lightPipelineStateCreationDesc));
    ...
    mFinalPipelineState =
        std::make_unique<gfx::PipelineState>(mDevice->CreatePipelineState(finalRenderPassPipelineStateCreationDesc));
    ...
    mSkyBoxPipelineState =
        std::make_unique<gfx::PipelineState>(mDevice->CreatePipelineState(skyBoxPipelineStateCreationDesc));
}
