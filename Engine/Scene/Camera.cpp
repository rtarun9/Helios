...
namespace helios::scene
{
	...
	void Camera::Update(float deltaTime)
	{
		...
		// The friction "smooth" or how slowly / fast you want these values to lerp to 0.0f or current frame value.
		...
		moveTo = math::XMVectorLerp(moveTo, cameraMoveToPosition, mFrictionFactor);

		cameraPosition += moveTo;

		pitchShift = std::lerp(pitchShift, pitchTo, mFrictionFactor);
		yawShift = std::lerp(yawShift, yawTo, mFrictionFactor * movementSpeed);
