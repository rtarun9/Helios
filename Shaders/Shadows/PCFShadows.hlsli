#ifndef __PCF_SHADOWS_HLSLI__
#define __PCF_SHADOWS_HLSLI__
...
float CalculateShadow(float4 lightSpaceWorldPosition, float nDotL, uint shadowDepthBufferIndex)
{
    ...
    for (int x = -samplesOffset; x <= samplesOffset; ++x)
    {
        ...
        {
            float currPCFDepth = shadowDepthBuffer.Sample(pointClampSampler, shadowPosition.xy + float2(x, y) * texelSize).r;
