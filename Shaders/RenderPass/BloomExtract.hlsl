...
{
	Texture2D<float4> inputShadingPassTexture = ResourceDescriptorHeap[renderResources.inputShadingPassTextureIndex];
	Texture2D<float4> inputLightPassTexture = ResourceDescriptorHeap[renderResources.inputLightPassTextureIndex];
	...
	const float3 color = inputShadingPassTexture.SampleLevel(linearClampToBorder, uvCoords, 0).xyz + inputLightPassTexture.SampleLevel(linearClampToBorder, uvCoords, 0).xyz;
