...
float PsMain(VSOutput input) : SV_Target
{
    ...
    const float2 noiseScale = float2((float)ssaoBuffer.screenWidth / (float)ssaoBuffer.noiseTextureWidth, (float)ssaoBuffer.screenHeight / (float)ssaoBuffer.noiseTextureHeight);

    const float3 randomVector = float3(randomRotationTexture.Sample(pointWrapSampler, input.textureCoord * noiseScale).xy, 0.0f);
    ...
    return pow(abs(1.0f - (ssaoBuffer.occlusionMultiplier * occlusion / (float)ssaoBuffer.sampleVectorCount)), ssaoBuffer.power);
}