...
namespace helios::gfx
{
    ...
    // D3D12MA is used rather than creation of committed resources. This is because Placed resources (which D3D12MA may allocate)
    // can be more memory efficient. Also, a lot of issues such as fragmentation are handled automatically by D3D12MA.
    // A 'Allocation' represents a GPU allocation (i.e represents a heap allocated from the GPU memory (based on the Heap Type, that is either on GPU only memory
    // or on the shared CPU / GPU memory)).
